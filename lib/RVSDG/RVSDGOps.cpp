...
GammaNode::verify()
{
  ...
  if (predicateType.getNumOptions() != this->getNumRegions())
  {
    return emitOpError("has predicate with wrong number of options. Expected ")
        << this->getNumRegions() << ", got " << predicateType.getNumOptions();
  }
...
GammaResult::verify()
{
  ...
  if (parent == NULL)
  {
    return emitOpError("GammaResult has no parent of type GammaNode. This error should never "
  ...
  for (size_t i = 0; i < results.size(); ++i)
  {
    ...
    {
      return emitError() << "type of output operand " << i << " (" << getOperand(i).getType()
                         << ") does not match node output type (" << results[i].getType() << ")";
    }
  }
...
mlir::Region *
LambdaNode::getCallableRegion()
{
  return &this->getRegion();
}

llvm::ArrayRef<mlir::Type>
LambdaNode::getArgumentTypes()
{
  // TODO - Implement this function
  std::cerr
      << "llvm::ArrayRef<mlir::Type> LambdaNode::getArgumentTypes() - has not been implemented"
      << std::endl;
  exit(-1);
  return {};
}
...
ApplyNode::verify()
{
  ...
  if (lambdaType.getParameterTypes().size() != paramTypes.size())
  {
    return this->emitOpError(" has the wrong number of parameters.")
        << " Lambda expects " << lambdaType.getParameterTypes().size() << " but "
        << paramTypes.size() << " were given.";
  }

  if (lambdaType.getParameterTypes().size() != paramTypes.size())
  {
    return this->emitOpError(" has the wrong number of result types.")
        << " Lambda provides " << lambdaType.getParameterTypes().size() << " but "
        << paramTypes.size() << " were specified.";
  }
...
ApplyNode::getCallableForCallee()
{
  return getLambda();
}

mlir::Operation::operand_range
ApplyNode::getArgOperands()
{
  return this->getOperands().drop_front();
}

void
ApplyNode::setCalleeFromCallable(CallInterfaceCallable callee)
{
  // TODO - Implement this function
  std::cerr << "void ApplyNode::setCalleeFromCallable(CallInterfaceCallable callee) - has not been "
               "implemented"
            << std::endl;
  exit(-1);
}
...
ApplyNode::getArgOperandsMutable()
{
  // TODO - Implement this function
...
ThetaNode::verify()
{
  ...
  if (inputTypes.size() != outputTypes.size() || inputTypes.size() != regionArgTypes.size())
  {
  ...
  size_t typeIndex = 0;
  for (auto [inType, outType, argType] : zip(inputTypes, outputTypes, regionArgTypes))
  {
    if (inType != outType || inType != argType)
    {
      return emitOpError("Type mismatch between node inputs, node outputs, and "
                         "region arguments. "
                         "Offending argument: #")
          << typeIndex << " Input type: " << inType << " Output type: " << outType
...
ThetaResult::verify()
{
  ...
  if (parent == NULL)
  {
    return emitOpError("ThetaResult has no parent of type ThetaNode. This error should never "
                       "appear, so if it does, may God have mercy on your soul");
  }
  ...
  for (auto [inType, resType] : zip(thetaOperandTypes, resultTypes))
  {
    if (inType != resType)
    {
      return emitOpError("Type mismatch between theta inputs and theta "
                         "result Op. Offending type: #")
          << typeIndex << " Input has type " << inType << ", result Op has type " << resType;
    }
    ++typeIndex;
  }
  ...
  for (size_t outputIndex = 0, argIndex = 0; outputIndex < nOutputs; ++outputIndex, ++argIndex)
  {
    if (this->getResult(outputIndex).getType() != this->getRegion().getArgument(argIndex).getType())
    {
      return this->emitOpError(" has a type mismatch between outputs and region arguments.")
          << "Argument #" << argIndex << " is of type "
          << this->getRegion().getArgument(argIndex).getType() << ", but output #" << outputIndex
          << " is of type " << this->getResult(outputIndex).getType();
    }
  }

  for (size_t operandIndex = 0, argIndex = nOutputs; operandIndex < nOperands;
       ++operandIndex, ++argIndex)
  {
    if (this->getOperand(operandIndex).getType()
        != this->getRegion().getArgument(argIndex).getType())
    {
      this->emitOpError(" has a type mismatch between operands and region arguments.")
          << "Argument #" << argIndex << " is of type "
          << this->getRegion().getArgument(argIndex).getType() << ", but operand #" << operandIndex
          << " is of type " << this->getOperand(operandIndex).getType();
    }
  }
...
PhiResult::verify()
{
  ...
  if (parent == NULL)
  {
    return emitOpError("PhiResult has no parent of type PhiNode. This error should never "
                       "appear, so if it does, may God have mercy on your soul");
  }
  auto resultTypes = this->getOperandTypes();
  ...
  for (auto [inType, argType] : zip(inputTypes, regionArgTypes))
  {
    if (inType != argType)
    {
      return emitOpError("Type mismatch between node inputs and region arguments.")
          << " Offending argument: #" << typeIndex << " Input type: " << inType
          << " Region argument type: " << argType;
    }
    ++typeIndex;
  }
  ...
  for (auto opaqueAttr : mappingAttr)
  {
    if (auto matchRuleAttr = opaqueAttr.dyn_cast<MatchRuleAttr>())
    {
      if (matchRuleAttr.isDefault())
      {
        ...
        {
          return emitOpError(
              "Match operator has more than one default rule in its mapping attribute.");
        }
        else
        {
          hasDefault = true;
      ...
      for (auto value : matchValues)
      {
        if (seenInputs.count(value) != 0)
        {
          return emitOpError(" has a duplicate input in its mapping attribute.")
              << " Input " << value << " in rule #" << ruleIndex << ". Previously seen in rule #"
              << seenInputs[value];
        }
      ...
      if (matchResult >= nOptions)
      {
        return emitOpError(" has a result index that is out of bounds in its mapping attribute.")
            << " Result index: " << matchResult << " Number of options: " << nOptions;
      }
      ruleIndex++;
    }
    else
    {
      return emitOpError(
          "Match operator has a non-MatchRuleAttr attribute in its mapping attribute.");
    }
  }
  return LogicalResult::success();
}
 ...
 **/
void
printTypedParamList(OpAsmPrinter & p, Operation * op, OperandRange operands, TypeRange types)
{
  p << "(";
 ...
 **/
void
printRVSDGRegion(OpAsmPrinter & p, Operation * op, Region & region)
{
  p << "(";
...
void
printRVSDGRegions(OpAsmPrinter & p, Operation * op, MutableArrayRef<Region> regions)
{
  p.increaseIndent();
  p << "[";
  p.printNewline();
...
ParseResult
parseRVSDGRegion(OpAsmParser & parser, Region & region)
{
  SmallVector<OpAsmParser::Argument, 4> arguments;
  if (failed(parser.parseArgumentList(arguments, OpAsmParser::Delimiter::Paren, true, true)))
  {
    return parser.emitError(parser.getCurrentLocation(), "Failed to parse argument list");
  }
  if (failed(parser.parseColon()))
  {
    return parser.emitError(parser.getCurrentLocation(), "Expected a \":\" token");
  }

  if (failed(parser.parseRegion(region, arguments, true)))
  {
    return parser.emitError(parser.getCurrentLocation(), "Failed to parse region");
  }
  return ParseResult::success();
}

ParseResult
parseRVSDGRegions(OpAsmParser & parser, SmallVectorImpl<std::unique_ptr<Region>> & regions)
{
  ...
  ParseResult result = parser.parseCommaSeparatedList(OpAsmParser::Delimiter::Square, parseRegion);
  if (failed(result))
  {
    return parser.emitError(parser.getCurrentLocation(), "Failed to parse region list");
  }
  return ParseResult::success();
}
 ...
 */
void
mlir::rvsdg::RVSDGDialect::addRVSDGOps()
{
  addOperations<
#define GET_OP_LIST
#include "RVSDG/Ops.cpp.inc"
